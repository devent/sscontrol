...
class RoundcubeProxyTest extends UbuntuTestUtil {
    ...
    void "roundcube reverse proxy"() {
        ...
        assertStringContent proxyTarOutExpected.replaced(tmpdir, tmpdir, "/tmp").replaceAll(/\d+-\d+-\d+T\d+-\d+-\d+-\d+/, "timestamp"), proxyTarOutExpected.toString()
        assertFileContent proxyGzipOutExpected.asFile(tmpdir), proxyGzipOutExpected
        assertFileContent proxyMysqldumpOutExpected.asFile(tmpdir), proxyMysqldumpOutExpected
